...
### Update the OCS operator SHAs
As of this writing the OCS operator is listing manifests digest instead of image SHAs which can be problematic, particularly when using the internally registry to mirror so we'll replace them.
