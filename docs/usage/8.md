...
## Introduction
This guide was develeped using the [generic instructions](https://docs.openshift.com/container-platform/4.2/operators/olm-restricted-networks.html) for mirroring operators from OLM. 

The steps in these instructions roughly correspond with the steps in the document linked above.
...
## Step 0b: To use the internal openshift registry you can expose and push to the internal registry.
```
...
### Update the OCS operator SHA's
As of this writing the ocs operator is listing manifests digest instead of image SHA's which can be problematic, particularly when using the internally registry to mirror so we'll replace them.
...
export new_roperator_sha=$(oc image mirror --dry-run --insecure=true ${remote_registry}/${remote_registry_ns}/${roperator_image}@sha256:${roperator_sha} ${exposed_registry}/${local_registry_ns}/rook-ceph-rhel8-operator:v4.2.0 2>&1 | grep -A1 manifests | grep sha256 | awk -F'[: ]' '{ print $8 }')
export new_ooperator_sha=$(oc image mirror --dry-run --insecure=true ${remote_registry}/${remote_registry_ns}/${ooperator_image}@sha256:${ooperator_sha} ${exposed_registry}/${local_registry_ns}/ocs-rhel8-operator:v4.2.0 2>&1 | grep -A1 manifests | grep sha256 | awk -F'[: ]' '{ print $8 }')
export new_moperator_sha=$(oc image mirror --dry-run --insecure=true ${remote_registry}/${remote_registry_ns}/${moperator_image}@sha256:${moperator_sha} ${exposed_registry}/${local_registry_ns}/mcg-rhel8-operator:v4.2.0 2>&1 | grep -A1 manifests | grep sha256 | awk -F'[: ]' '{ print $8 }')
export new_noobaa_sha=$(oc image mirror --dry-run --insecure=true ${remote_registry}/${remote_registry_ns}/${noobaa_image}@sha256:${noobaa_sha} ${exposed_registry}/${local_registry_ns}/mcg-core-rhel8:v4.2.0 2>&1 | grep -A1 manifests | grep sha256 | awk -F'[: ]' '{ print $8 }')
sed -i "s/${roperator_sha}/${new_roperator_sha}/g" manifests/ocs-operator/4.2.0/ocs-operator.v4.2.0.clusterserviceversion.yaml
sed -i "s/${moperator_sha}/${new_moperator_sha}/g" manifests/ocs-operator/4.2.0/ocs-operator.v4.2.0.clusterserviceversion.yaml
sed -i "s/${ooperator_sha}/${new_ooperator_sha}/g" manifests/ocs-operator/4.2.0/ocs-operator.v4.2.0.clusterserviceversion.yaml
sed -i "s/${noobaa_sha}/${new_noobaa_sha}/g" manifests/ocs-operator/4.2.0/ocs-operator.v4.2.0.clusterserviceversion.yaml
```

### Update the OCS operator source and namespaces for dev/stage
host unknown errors seem to stop the image mirror policy from working? The same with namespace and permissions to it? These claims need verification.
```
...
## Step 6a: Write the Dockerfile for the custom registry
```
cat << EOF > Dockerfile
...
## Step 8a: Create openshift-migration Namespace and Catalog Source
```
...
## Step 8b: Install the Catalog Source
```
...
## Stepb 9b: Create the ImageContentSourcePolcy for each image
```
