...
Run the following as a pre-backup hook on the destination. The migration-controller CR in the openshift-migration namespace is capable of running these tasks.
```
- hosts: localhost
  gather_facts: false
  vars:
