...
Manifests for deploying mig-operator to OpenShift 3 and 4 have been made available. Choose the appropriate manifest version for each OpenShift cluster you install mig-operator onto.
- [operator-3.yml](https://raw.githubusercontent.com/fusor/mig-operator/master/operator-3.yml)
- [operator-4.yml](https://raw.githubusercontent.com/fusor/mig-operator/master/operator-4.yml)
...
# Note we are using the OpenShift 3 manifest here.
oc create -f https://raw.githubusercontent.com/fusor/mig-operator/master/operator-3.yml
...
# Create mig-operator (requires cluster-admin privilege). 
# Note we are using the OpenShift 4 manifest here.
oc create -f https://raw.githubusercontent.com/fusor/mig-operator/master/operator-4.yml
```
...
3. Install NooBaa, take note of the output values for 'AWS_ACCESS_KEY_ID' and 'AWS_SECRET_ACCESS_KEY' that will be produced for a later step. Also note that an initial bucket has been created called 'first.bucket'
```
...
```
$ AWS_ACCESS_KEY_ID="ygeJ5GzAwbBJiSukw8Lv" AWS_SECRET_ACCESS_KEY="so2C5X/ttRhiX00DZrOnv0MxV0r5VlOkYmptTU91" aws s3 ls --endpoint http://s3-noobaa.apps.destcluster.com
