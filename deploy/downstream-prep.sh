...
for f in deploy/olm-catalog/bundle/manifests/konveyor-operator.${MTCVERSION}.clusterserviceversion.yaml \
  ...
  sed -i "s,/log-reader:.*,/openshift-migration-log-reader-rhel7@sha256:${IMG_MAP[logreader_sha]},g"                                                                 ${f}
  ...
  sed -i "s,log-reader-rhel7@sha256:.*,log-reader-rhel7@sha256:${IMG_MAP[logreader_sha]},g"                                                                          ${f}
  sed -i 's,value: log-reader,value: openshift-migration-log-reader-rhel7@sha256,g'                                                                                  ${f}
